...
- **Quỹ Mạnh** — của riêng chồng, em cũng không thấy.
...
## Lần đầu dùng — 4 bước setup (~10 phút)
...
### Bước 3. Nhập số dư khởi điểm của 3 quỹ
...
Vào **⚙️ Cài đặt** → khối **Số dư khởi điểm**:
...
Em **không cần** nhập Quỹ Mạnh — chồng tự nhập của chồng.
...
### Bước 4. Set quy tắc lương (nếu em có lương cố định)

Cùng khối **⚙️ Cài đặt** → tìm phần quy tắc lương. Quy tắc trả lời câu hỏi:

> *"Mỗi lần lương của em về, sẽ chia mấy % vào Quỹ Vợ, mấy % vào Quỹ Chung?"*

Ví dụ:
- Em muốn giữ 70% cho cá nhân, đóng 30% vào quỹ chung → set **`70 / 30`**.
- Hoặc đóng cố định 5tr/tháng vào quỹ chung, phần còn lại về quỹ riêng → set **`100 / 0` + cố định 5,000,000 vào quỹ chung**.

Quy tắc này dùng cho sau này khi em log lương qua chat — agent sẽ tự chia.

> ⚠️ **Tổng % phải = 100**. App sẽ báo lỗi nếu sai.
...
> 📊 **Cách tính**: tiến độ trên dashboard = (tổng thu - tổng chi của cả 3 quỹ, đã loại các giao dịch chuyển nội bộ giữa các quỹ).

### Quỹ mục tiêu (envelope)

Nếu vợ chồng có nhiều mục tiêu nhỏ rõ ràng (vd: du lịch hè, sửa nhà, học phí cho con, đầu tư), tạo **Quỹ mục tiêu** riêng cho từng cái.

Vào tab **🎯 Mục tiêu** ở sidebar → **+ Tạo quỹ**. Mỗi quỹ điền:

- **Tên**: vd "Quỹ Du lịch Đà Nẵng", "Quỹ Mua xe", "Quỹ Bin học cấp 1"
- **Mục tiêu**: tổng tiền cần (vd `50000000` = 50tr)
- **Deadline** (optional): ngày cần đạt (vd `2026-09-15`)
- **Đóng góp/tháng** (optional): tháng nào cũng cần bỏ vào ít nhất bao nhiêu (vd `5000000` = 5tr/tháng)
...
- **Mục tiêu năm** — em đã tiết kiệm được bao nhiêu, đang vượt/đúng/chậm tiến độ
- **Quỹ mục tiêu** — tiến độ từng envelope (du lịch, mua xe…)
- **Tháng này** — thu / chi / net
- **Số dư 3 quỹ** (riêng Quỹ Mạnh em sẽ thấy `— — — đ` vì privacy)
...
Quản lý các quỹ mục tiêu (envelope) đã setup ở trên — sửa, archive, unarchive.

### ⚙️ Cài đặt

Tài khoản, đổi mật khẩu, mục tiêu năm, số dư khởi điểm, quy tắc lương.

---

## Privacy — đọc kỹ phần này 🔒

- **Quỹ Mạnh em không bao giờ thấy** số dư hay giao dịch. Trong sidebar nó hiện `🔒 Riêng tư — — — đ`.
...
- AI agent có "thấy" toàn bộ 3 quỹ để đưa lời khuyên tài chính cho cặp đôi (vd "tháng này 2 vợ chồng tiêu hơi cao mảng ăn ngoài"), **nhưng** trong câu trả lời nó sẽ KHÔNG bao giờ nói tên giao dịch hay số tiền cụ thể của Quỹ Mạnh / Quỹ Vợ — chỉ aggregate ẩn danh.
...
| Tạo quỹ mới (du lịch, đầu tư…) | 🎯 Mục tiêu → + Tạo quỹ |
| Báo cáo thu/chi tháng | 📈 Báo cáo |
| Đổi mật khẩu | ⚙️ Cài đặt → Tài khoản |
| Đổi quy tắc chia lương | ⚙️ Cài đặt |
