...
function YearlyGoalSection() {
  ...
  return (
    <Card padding="p-6">
      <h3 className="mb-1 text-sm font-semibold text-stone-800">Mục tiêu tiết kiệm năm {year}</h3>
      ...
      {!loading && (
        <>
          <label className="mb-1.5 block text-xs font-medium text-stone-700">Mục tiêu (VND)</label>
          <input
            type="number"
            value={target}
          ...
          <p className="mt-1 text-[11px] text-stone-400">
            {target !== '' && Number(target) > 0
              ? <><span>Hiển thị: </span><span className="font-mono">{formatVND(Number(target))}</span></>
...
function OpeningBalanceSection() {
  ...
  return (
    <Card padding="p-6">
      <h3 className="mb-1 text-sm font-semibold text-stone-800">Số dư khởi đầu</h3>
      <p className="mb-5 text-xs text-stone-500">
        Tiền đã có sẵn trong mỗi quỹ tại thời điểm bạn bắt đầu xài Concord
        (vd: số dư app ngân hàng, ví Shopee, tiền mặt). Khoản này được ghi như 1 entry cấu trúc —
        không hiện trong &ldquo;Giao dịch gần đây&rdquo; và không tính vào báo cáo thu/chi tháng.
        Số dư khởi đầu của quỹ tiết kiệm &amp; đầu tư{' '}
    ...
    <div className={`rounded-lg border px-4 py-3 ${isJoint ? 'border-amber-100 bg-amber-50/40' : 'border-emerald-100 bg-emerald-50/30'}`}>
      <div className="mb-2 flex items-baseline justify-between">
        ...
        <span className="text-[11px] text-stone-500">
          Số dư hiện tại: <span className="font-mono">{formatVND(fund.balance ?? 0)}</span>
        </span>
      </div>
